...
@AndroidEntryPoint
class FragmentAllQuestion : BaseFragment<FragmentAllQuestionBinding, FragmentAllQuestionViewModel>(FragmentAllQuestionBinding::inflate),
    ItemOnClickListener {
    ...
    override fun showAnotherFragment(item: ItemUi) {
        viewModel.goQuestionEditFragment(question = viewModel.adapterMapper.map(item as QuestionModel), title = title, id = id, chapter = chapter)
    }
    ...
    override fun deleteItem(item: ItemUi, position: Int) {
        ...
        val dialog = AlertDialog.Builder(requireContext()).setTitle(R.string.login_out_title).setCancelable(true)
            .setMessage(R.string.default_delete_question_alert_message).setPositiveButton(R.string.action_yes, listener)
