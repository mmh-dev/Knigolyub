...
class FragmentClassUsersViewModel @Inject constructor(
    ...
    fun fetchUsers(classId: String) = launchInBackground {
        getClassUsersUseCase.execute(classId = classId).collectLatest { resource ->
    ...
    private fun unravelingResource(
        ...
        when (resource.status) {
            ...
            Status.ERROR -> communication.put(listOf(ErrorModel(resource.message!!) { fetchUsers(classId = classId) }))
    ...
    fun goUserDetailsFragment(student: Student) =
        navigate(FragmentClassUsersDirections.actionFragmentClassStudentsToFragmentStudentDetails2(student = student))
