...
@AndroidEntryPoint
class FragmentChapterBook : BaseFragment<FragmentChapterBookBinding, FragmentStudentChapterBookViewModel>(FragmentChapterBookBinding::inflate),
    ChapterItemOnClickListener, OnLoadCompleteListener, OnErrorListener, OnPageChangeListener {
    ...
    private fun loadSuccess() {
        binding().pdfView.fromFile(File(path)).onPageChange(this@FragmentChapterBook).onError(this@FragmentChapterBook)
            .onLoad(this@FragmentChapterBook).load()
    }
    ...
    override fun goReadPdfFragment(chapter: PdfDocument.Bookmark, position: Int) {
        ...
        viewModel.goReaderFragment(book = book, chapter = position + 1, startPage = chapter.pageIdx.toInt(), lastPage = lastPage, path = path)
    }
