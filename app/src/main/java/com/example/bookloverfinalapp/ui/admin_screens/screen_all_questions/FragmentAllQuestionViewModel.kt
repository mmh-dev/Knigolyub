...
import com.example.bookloverfinalapp.app.ui.adapter.QuestionLoadingModel
...
class FragmentAllQuestionViewModel @Inject constructor(
    ...
    fun collect(owner: LifecycleOwner, observer: Observer<List<ItemUi>>) = communication.observe(owner = owner, observer = observer)

    fun fetchBookQuestions(id: String, chapter: Int) = launchInBackground {
        getAllChapterQuestionsUseCase.execute(id = id, chapter = chapter).collectLatest { resource ->
    ...
    private fun unravelingResource(
        ...
        when (resource.status) {
            ...
            Status.ERROR -> communication.put(listOf(ErrorModel(resource.message!!) { fetchBookQuestions(id = id, chapter = chapter) }))

            Status.EMPTY -> communication.put(listOf(CubeEmptyModel))
        }
    }

    fun deleteQuestion(id: String) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
    ...
    fun goAddQuestionFragment(id: String, chapter: Int, title: String) =
        navigate(FragmentAllQuestionDirections.actionFragmentAllQuestionToFragmentAddQuestion(id = id, title = title, chapter = chapter))
    ...
    fun goQuestionEditFragment(
        ...
        chapter: Int,
        id: String,
    ) = navigate(FragmentAllQuestionDirections.actionFragmentAllQuestionToFragmenAdminEditQuestion(question = question,
        title = title,
        chapter = chapter,
        id = id))
