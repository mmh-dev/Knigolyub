...
@AndroidEntryPoint
class FragmentMyBooks : BaseFragment<FragmentMyBooksBinding, MyBooksViewModel>(FragmentMyBooksBinding::inflate), SwipeRefreshLayout.OnRefreshListener,
    ItemOnClickListener {
    ...
    private fun observeResource() {
        ...
        viewModel.bookObserve(viewLifecycleOwner) { books ->
            books.forEach { book ->
                ...
                if (path == null) viewModel.getBookPdf(book.book).observe(viewLifecycleOwner) { inputStream ->
    ...
    override fun showAnotherFragment(item: ItemUi) {
        ...
        else findNavController().navigate(FragmentRootStudentBookDirections.actionFragmentRootBookToFragmentChapterBook(book = viewModel.adapterMapper.map(
                book), path = path))
    }
    ...
    override fun deleteItem(item: ItemUi, position: Int) {
        ...
        val listener = DialogInterface.OnClickListener { _, which ->
            when (which) {
                DialogInterface.BUTTON_POSITIVE -> viewModel.deleteBook(id = id).observe(viewLifecycleOwner) {
        ...
        val dialog = AlertDialog.Builder(requireContext()).setCancelable(true).setMessage(R.string.default_delete_alert_message)
            .setPositiveButton(R.string.action_yes, listener).setNegativeButton(R.string.action_no, listener)
            .setNeutralButton(R.string.action_ignore, listener).setOnCancelListener { showToast(R.string.cancle_delete) }.create()
