...
class FragmentBookQuestionViewModel @Inject constructor(
    ...
    fun getAllChapterQuestions(
        id: String,
        chapter: Int,
    ): StateFlow<Resource<List<BookQuestionDomain>>> = getAllChapterQuestionsUseCase.execute(id = id, chapter = chapter)
        .stateIn(scope = viewModelScope, started = SharingStarted.Lazily, initialValue = Resource.loading())

    fun goChapterFragment(book: BookThatRead, path: String) {
        navigate(FragmentBookQuestionDirections.actionFragmentBookQuestionToFragmentChapterBook(book = book, path = path))
    }

    fun updateChapters(
        id: String, chapters: Int, isReadingPages: List<Boolean>, book: BookThatRead, path: String,
    ) {
        updateChaptersUseCase.execute(id = id, isReadingPages = isReadingPages, chapters = chapters).onEach { resource ->
