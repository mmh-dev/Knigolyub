...
class FragmentClassesViewModel @Inject constructor(
    ...
    fun classesCollect(owner: LifecycleOwner, observer: Observer<List<SchoolClass>>) = classCommunication.observe(owner = owner, observer = observer)
    ...
    private fun unravelingResource(
        ...
        when (resource.status) {
            ...
            Status.ERROR -> communication.put(listOf(ErrorModel(resource.message!!) { fetchClasses(schoolId = schoolId) }))
        }
    }

    fun deleteClass(id: String) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
    ...
    fun addNewClass(title: String, schoolId: String) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
        addNewClassUseCase.execute(title = title, schoolId = schoolId).collectLatest { resource ->
                when (resource.status) {
                    Status.LOADING -> showProgressDialog()
                    Status.SUCCESS -> {
                        dismissProgressDialog()
    ...
    fun goClassStudentFragment(schoolClass: SchoolClass) =
        navigate(FragmentAdminMainRootDirections.actionFragmentAdminMainRootToFragmentClassStudents(schoolClass))
}