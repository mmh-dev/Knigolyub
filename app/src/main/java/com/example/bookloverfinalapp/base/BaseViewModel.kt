...
abstract class BaseViewModel : ViewModel() {
    ...
    fun collectError(owner: LifecycleOwner, observer: Observer<Event<String>>) = errorCommunication.observe(owner = owner, observer = observer)

    fun navigate(navDirections: NavDirections) = navigationCommunication.put(Event(NavigationCommand.ToDirection(navDirections)))

    fun navigateBack() = launchInBackground { navigationCommunication.put(Event(value = NavigationCommand.Back)) }

    fun error(message: String) = launchInBackground { errorCommunication.put(Event(value = message)) }

    fun showProgressAnimation() = launchInBackground { progressCommunication.put(Event(value = true)) }

    fun dismissProgressAnimation() = launchInBackground { progressCommunication.put(Event(value = false)) }

    fun showProgressDialog() = launchInBackground { progressDialogCommunication.put(true) }

    fun dismissProgressDialog() = launchInBackground { progressDialogCommunication.put(false) }

    fun <T> launchInBackground(backgroundCall: suspend () -> T) = dispatchers.launchInBackground(viewModelScope) { backgroundCall() }
}
