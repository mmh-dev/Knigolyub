...
@AndroidEntryPoint
class FragmentAdminUploadPdf : BaseFragment<FragmentAdminUploadPdfBinding, FragmentAdminUploadPdfViewModel>(FragmentAdminUploadPdfBinding::inflate),
    OnPageChangeListener, OnLoadCompleteListener, OnErrorListener, View.OnClickListener {
    ...
    private fun pickFile() {
        val permissionCheck = ContextCompat.checkSelfPermission(requireContext(), READ_EXTERNAL_STORAGE)
    ...
    private fun displayFromUri(uri: Uri) {
        binding().pdfViewAdmin.fromUri(uri).defaultPage(0).onPageChange(this).enableAnnotationRendering(true).onLoad(this)
            .scrollHandle(DefaultScrollHandle(requireContext())).onError(this).load()
    }
    ...
    private fun generateImageFromPdf(pdfUri: Uri) = lifecycleScope.launch(Dispatchers.IO) {
        ...
        try {
            val fd: ParcelFileDescriptor = requireActivity().contentResolver.openFileDescriptor(pdfUri, "r")!!
            ...
            withContext(Dispatchers.Main) {
                requireContext().glide(bmp, binding().roundedBookImage)
            }
        } catch (e: Exception) {
            //todo with exception
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == PERMISSION_CODE && grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) launchPicker()
    ...
    private fun showGenresSingleChoiceWithConfirmationAlertDialog() {
        val dialog = AlertDialog.Builder(requireContext()).setSingleChoiceItems(allBookGenresList.toTypedArray(), bookGenresCurrentIndex, null)
