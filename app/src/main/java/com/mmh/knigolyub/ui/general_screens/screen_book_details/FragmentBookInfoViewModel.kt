...
class FragmentBookInfoViewModel @Inject constructor(
    ...
    fun chekIsMyBook(id: String, userId: String) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
    ...
    fun fetchSimilarBook(genres: List<String>, bookId: String) = launchInBackground {
        getSimilarBooksUseCase.execute(genres = genres, bookId = bookId).collectLatest { resource ->
                unravelingResource(resource = resource, genres = genres, bookId = bookId)
            }
    }
    ...
    private fun unravelingResource(
        ...
        when (resource.status) {
            ...
            Status.ERROR -> communication.put(listOf(ErrorModel(resource.message!!) {
                fetchSimilarBook(bookId = bookId, genres = genres)
            }))
        }
    }
    ...
    fun goGenreSimilarBooks(genre: String) = navigate(FragmentBookInfoDirections.actionFragmentBookInfoToFragmentGenreSimilarBooks(genre = genre))
