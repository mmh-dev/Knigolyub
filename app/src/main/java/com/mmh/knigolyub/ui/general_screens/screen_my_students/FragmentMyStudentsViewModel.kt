    ...
    private fun unravelingResource(resource: Resource<List<StudentDomain>>, classId: String) {
        when (resource.status) {
            ...
            Status.SUCCESS -> communication.put(resource.data!!.map { studentDomain ->
                StudentDomainToUserModelMapper(ADAPTER_USER_VIEW_HOLDER).map(studentDomain)
            })
            Status.EMPTY -> communication.put(listOf(CatEmptyModel))
            Status.ERROR -> communication.put(listOf(ErrorModel(resource.message!!) { onRefresh(classId = classId) }))
        }
    }

    fun goStudentDetailsFragment(student: Student) =
        navigate(FragmentRootStudentBookDirections.actionFragmentRootBookToFragmentStudentDetails(student = student))
}