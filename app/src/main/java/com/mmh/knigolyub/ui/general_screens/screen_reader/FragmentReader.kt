...
@AndroidEntryPoint
class FragmentReader : BaseFragment<FragmentReaderBinding, FragmentReaderViewModel>(FragmentReaderBinding::inflate), OnLoadCompleteListener,
    OnErrorListener, OnPageChangeListener, View.OnClickListener {
    ...
    private fun loadSuccess() {
        ...
        binding().pdfview.fromFile(File(path)).spacing(100).pages(*pdfPages.toIntArray()).swipeHorizontal(true).enableDoubletap(true)
            .onPageChange(this).onLoad(this).enableAnnotationRendering(true).password(null).scrollHandle(null).load()
    }
    ...
    override fun onClick(view: View?) {
        when (view) {
            binding().endRead -> {
                if (NavigationManager().isOnline(context = requireContext())) viewModel.goQuestionFragment(book = book,
                    chapter = chapter,
                    path = path)
    ...
    private fun showCustomInputAlertDialog(text: String) {
        ...
        val dialog =
            AlertDialog.Builder(requireContext()).setTitle(dialogTitle).setView(dialogBinding.root).setPositiveButton(R.string.action_confirm, null)
                .create()
                ...
                if (enteredText.isBlank()) {
                    ...
                    if (newPage in startPage..lastPage) {
                        pages.forEach { oldPage ->
                            if (oldPage == newPage) binding().pdfview.jumpTo(pages.indexOf(oldPage))
                        }
                        dialog.dismiss()
