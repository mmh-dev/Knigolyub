...
@AndroidEntryPoint
class FragmentClasses : BaseFragment<FragmentClassesBinding, FragmentClassesViewModel>(FragmentClassesBinding::inflate), ItemOnClickListener {
    ...
    private fun showCustomAlertDialog() {
        ...
        val dialog = AlertDialog.Builder(requireContext()).setTitle(R.string.add_class).setView(dialogBinding.root)
            .setPositiveButton(R.string.action_confirm, null).setOnCancelListener {
    ...
    private fun addClass(newSchoolClass: String) {
        ...
        if (counter == allClassesList.size) {
            viewModel.addNewClass(title = newSchoolClass, schoolId = currentUser.schoolId).observe(viewLifecycleOwner) {
                showToast(R.string.class_added_successfully)
                allClassesList.add(SchoolClass(id = it, title = newSchoolClass, schoolId = currentUser.schoolId))

                adapter.addItem(SchoolClassModel(id = it, title = newSchoolClass, schoolId = currentUser.schoolId))
    ...
    private fun showNumbersSingleChoiceWithConfirmationAlertDialog(textView: TextView) {
        val dialog = AlertDialog.Builder(requireContext())
            .setSingleChoiceItems(classNumbers.map { it.toString() }.toTypedArray(), classNumberCurrentIndex, null)
    ...
    private fun showTypesSingleChoiceWithConfirmationAlertDialog(textView: TextView) {
        val dialog = AlertDialog.Builder(requireContext()).setSingleChoiceItems(classTypes.toTypedArray(), classTypeCurrentIndex, null)
            .setPositiveButton(R.string.action_confirm) { d, _ ->
                val index = (d as AlertDialog).listView.checkedItemPosition
    ...
    override fun showAnotherFragment(item: ItemUi) {
        ...
        viewModel.goClassStudentFragment(schoolClass = SchoolClass(id = classUi.id, schoolId = classUi.schoolId, title = classUi.title))
    ...
    override fun deleteItem(item: ItemUi, position: Int) {
        ...
        val dialog = AlertDialog.Builder(requireContext()).setTitle(R.string.login_out_title).setCancelable(true)
            .setMessage(R.string.default_delete_class_alert_message).setPositiveButton(R.string.action_yes, listener)
            .setNeutralButton(R.string.action_ignore, listener).create()
