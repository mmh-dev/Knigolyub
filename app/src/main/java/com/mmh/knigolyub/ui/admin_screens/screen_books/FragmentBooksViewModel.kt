...
class FragmentBooksViewModel @Inject constructor(
    ...
    fun booksCollect(owner: LifecycleOwner, observer: Observer<List<Book>>) = bookCommunication.observe(owner = owner, observer = observer)
        ...
        when (resource.status) {
            ...
            Status.SUCCESS -> {
                communication.put(resource.data!!.map { bookDomain ->
                    BookDomainToBookModelMapper(ADAPTER_ADMIN_BOOK_VIEW_HOLDER).map(bookDomain)
                })
            ...
            Status.ERROR -> communication.put(listOf(ErrorModel(resource.message!!) { fetchBooks(schoolId = schoolId) }))
        }
    }

    fun searchBook(searchText: String, schoolId: String) = launchInBackground {
        getSearchBookUseCase.execute(schoolId = schoolId, searchText = searchText).collectLatest { resource ->
                unravelingResource(resource = resource, schoolId = schoolId)
            }
    }


    fun deleteBook(id: String) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
    ...
    fun updateBook(id: String, book: UpdateBookDomain) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
    ...
    fun getBookPdf(url: String) = liveData(context = viewModelScope.coroutineContext + dispatchersProvider.io()) {
    ...
    fun goChapterFragment(book: Book, path: String) =
        navigate(FragmentAdminMainRootDirections.actionFragmentAdminMainRootToFragmentAdminChapters(book = book, path = path))
