...
class FragmentSchoolProgressViewModel @Inject constructor(
    ...
    fun studentsCollect(owner: LifecycleOwner, observer: Observer<List<Student>>) = studentsCommunication.observe(owner = owner, observer = observer)
    ...
    private fun fetchStudents(allBooks: String, list: List<ClassDomain>) = launchInBackground {
        ...
        list.forEach { classDomain ->
            getClassUsersUseCase.execute(classId = classDomain.id).collectLatest { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
