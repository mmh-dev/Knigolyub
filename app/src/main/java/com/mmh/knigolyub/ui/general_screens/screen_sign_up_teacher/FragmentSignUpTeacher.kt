...
@AndroidEntryPoint
class FragmentSignUpTeacher : BaseFragment<FragmentSignUpTeacherBinding, FragmentSignUpViewModel>(FragmentSignUpTeacherBinding::inflate),
    ...
    private fun signUp() {
        binding().apply {
            if (!lastNameField.validateLastName()) showToast(message = getString(R.string.last_name_input_format_error))
                ...
                val teacherDto = UserSignUpDomain(name = firstNameField.text.toString().trim(),
                ...
                viewModel.signUp(user = teacherDto).observe(viewLifecycleOwner) { user ->
                    successSignUp(id = user.id, createdAt = user.createdAt, sessionToken = user.sessionToken, image = user.image)
                }
            }
        }
    }
    ...
    private fun showClassSingleChoiceWithConfirmationAlertDialog(list: List<String>) {
        val dialog =
            AlertDialog.Builder(requireContext()).setTitle(R.string.class_setup).setSingleChoiceItems(list.toTypedArray(), classCurrentIndex, null)
    ...
    private fun showSchoolSingleChoiceWithConfirmationAlertDialog(list: List<String>) {
        val dialog = AlertDialog.Builder(requireContext()).setTitle(R.string.volume_school_setup)
            .setSingleChoiceItems(list.toTypedArray(), schoolCurrentIndex, null).setPositiveButton(R.string.action_confirm) { d, _ ->
                val index = (d as AlertDialog).listView.checkedItemPosition
