...
class FragmentProgressViewModel @Inject constructor(
    ...
    private fun unravelingSchoolsStudents(
        ...
        when (resource.status) {
            ...
            Status.SUCCESS -> studentCommunication.put(resource.data!!.sortedByDescending { it.progress }.map { student ->
            ...
            Status.ERROR -> studentCommunication.put(listOf(ErrorModel(resource.message!!) { fetchSchoolStudents(schoolId = schoolId) }))
        }
    }
    ...
    private fun unravelingClassStudents(resource: Resource<List<StudentDomain>>, classId: String) {
        when (resource.status) {
            ...
            Status.SUCCESS -> {
                ...
                classStatisticsCommunication.put(ClassStatistics(readPages, readChapters, readBooks))

                studentCommunication.put(resource.data!!.sortedByDescending { it.progress }.map { studentDomain ->
                        StudentDomainToUserModelMapper(ADAPTER_USER_RATING_VIEW_HOLDER).map(studentDomain)
                    })
                dismissProgressAnimation()
            ...
            Status.ERROR -> {
                studentCommunication.put(listOf(ErrorModel(resource.message!!) { fetchMyStudent(classId) }))
                error(message = resource.message!!)
                dismissProgressAnimation()
            }
        }
