...
@AndroidEntryPoint
class FragmentBookInfo : BaseFragment<FragmentBookInfoBinding, FragmentBookInfoViewModel>(FragmentBookInfoBinding::inflate), View.OnClickListener,
    ...
    private fun addMyBooks() {
        ...
        val book = AddNewBookModel(bookId = book.objectId,
            page = book.page,
    ...
    private fun observeResource() {
        if (NavigationManager().isOnline(requireContext())) viewModel.chekIsMyBook(id = book.objectId, userId = currentUser.id)
        ...
        book.genres.forEach { genreCode ->
            binding().bookGenresLayout.addView(GenreTags(context = requireContext(),
                actionListener = this).getGenreTag(genreName = GenresManager.getGenreByCloudCode(context = requireContext(), code = genreCode),
                genreCode = genreCode))
        }
