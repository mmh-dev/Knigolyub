...
@AndroidEntryPoint
class FragmentBooks : BaseFragment<FragmentAdminBooksBinding, FragmentBooksViewModel>(FragmentAdminBooksBinding::inflate), ItemOnClickListener,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.booksCollect(viewLifecycleOwner) { books ->
            books.forEach { book ->
                ...
                if (path == null) viewModel.getBookPdf(book.book.url).observe(viewLifecycleOwner) { inputStream ->
    ...
    override fun updateItem(item: ItemUi, position: Int) {
        ...
        val dialog = AlertDialog.Builder(requireContext()).setView(dialogBinding!!.root).setPositiveButton(R.string.action_update, null)
            .setNeutralButton(R.string.action_ignore, null).setOnCancelListener { dialogBinding = null }.create()
        ...
        dialog.setOnShowListener {
            ...
            dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
                dialogBinding?.apply {
                    if (editTextBookAutor.text.toString().trim() == book.author && editTextBookPublicYear.text.toString()
                            .trim() == book.publicYear && editTextBookTitle.text.toString().trim() == book.title
    ...
    private fun updatedBook(book: Book, position: Int) {
        dialogBinding?.apply {
            ...
            val poster: BookPosterDomain = if (bookPoster == null) BookPosterDomain(url = book.poster.url, name = book.poster.name)
            else bookPoster!!
            val updateBook = UpdateBookDomain(title = title, author = author, poster = poster, publicYear = publicYear)

            viewModel.updateBook(id = book.objectId, book = updateBook).observe(viewLifecycleOwner) {
                    val newBook = BookModel(author = author,
                        ...
                        id = book.objectId,
                        book = BookModel.BookPdfAdapter(url = book.book.url, name = book.book.name, type = "File"),
                        title = title,
                        genres = book.genres,
    ...
    override fun deleteItem(item: ItemUi, position: Int) {
        ...
        val dialog =
            AlertDialog.Builder(requireContext()).setTitle(R.string.delete).setCancelable(true).setMessage(R.string.default_book_delete_alert_message)
                .setPositiveButton(R.string.action_yes, listener).setNeutralButton(R.string.action_ignore, listener).create()
