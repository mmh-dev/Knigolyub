...
class BookViewHolder(private val view: View, private val actionListener: ItemOnClickListener) : GenericViewHolder(view) {
    ...
    private fun showPopupMenu(view: View, position: Int) {
        ...
        popupMenu.menu.add(ID_BOOK_DELETE, ID_BOOK_DELETE, Menu.NONE, context.getString(R.string.delete))
        popupMenu.menu.add(ID_BOOK_UPDATE, ID_BOOK_UPDATE, Menu.NONE, context.getString(R.string.more_detailed))
...
class ErrorViewHolder(private val view: View) : GenericViewHolder(view) {
    override fun bind(item: ItemUi) {
        ItemFailFullscreenBinding.bind(view).apply {
            item.show(messageTextView, tryAgain, context = itemView.context, position = adapterPosition)
        }
    }
}
...
class UserTypeModelViewHolder(private val view: View) : GenericViewHolder(view) {
    override fun bind(item: ItemUi) {
        item.show(view.findViewById<MyTextView>(R.id.user_type_text), context = itemView.context, position = adapterPosition)
    }
}
...
class QuestionModelViewHolder(view: View, private val actionListener: ItemOnClickListener) : GenericViewHolder(view) {
...
class SchoolClassModelViewHolder(view: View, private val actionListener: ItemOnClickListener) : GenericViewHolder(view) {
    private val binding = ItemQuestionBinding.bind(view)
    override fun bind(item: ItemUi) {
        binding.apply {
            item.show(chapterText, context = itemView.context, position = adapterPosition)
...
class ViewHolderChain {
    ...
    fun execute(
        ...
        ADAPTER_BOOK_VIEW_HOLDER -> BookViewHolder(R.layout.item_book.makeView(parent = parent), actionListener = actionListener)

        ADAPTER_ADMIN_BOOK_VIEW_HOLDER -> BookViewHolder(R.layout.item_book.makeView(parent = parent), actionListener = actionListener)

        ADAPTER_SIMILAR_BOOK_VIEW_HOLDER -> SimilarBookViewHolder(R.layout.item_similar_book.makeView(parent = parent),
            actionListener = actionListener)

        ADAPTER_BOOK_THAT_READ_VIEW_HOLDER -> BookThatReadViewHolder(R.layout.item_my_book.makeView(parent = parent), actionListener = actionListener)

        ADAPTER_USER_RATING_VIEW_HOLDER -> UserRatingModelViewHolder(R.layout.item_student_rating.makeView(parent = parent),
            actionListener = actionListener)

        ADAPTER_USER_VIEW_HOLDER -> UserModelViewHolder(R.layout.item_user.makeView(parent = parent), actionListener = actionListener)

        ADAPTER_QUESTION_VIEW_HOLDER -> QuestionModelViewHolder(R.layout.item_question.makeView(parent = parent), actionListener = actionListener)

        ADAPTER_SCHOOL_CLASS_VIEW_HOLDER -> SchoolClassModelViewHolder(R.layout.item_question.makeView(parent = parent),
            actionListener = actionListener)

        ADAPTER_USER_TYPE_VIEW_HOLDER -> UserTypeModelViewHolder(R.layout.item_user_type.makeView(parent = parent))

        ADAPTER_ERROR_VIEW_HOLDER -> ErrorViewHolder(R.layout.item_fail_fullscreen.makeView(parent = parent))

        ADAPTER_USER_LOADING_VIEW_HOLDER -> UserLoadingViewHolder(R.layout.shimmer_my_student.makeView(parent = parent))

        ADAPTER_QUESTION_LOADING_VIEW_HOLDER -> QuestionLoadingViewHolder(R.layout.shimmer_class.makeView(parent = parent))

        ADAPTER_SIMILAR_BOOK_LOADING_VIEW_HOLDER -> SimilarBookLoadingViewHolder(R.layout.shimmer_similar_book.makeView(parent = parent))

        ADAPTER_BOOK_LOADING_VIEW_HOLDER -> BookLoadingViewHolder(R.layout.shimmer_book.makeView(parent = parent))

        ADAPTER_BOOK_THAT_READ_LOADING_VIEW_HOLDER -> BookThatReadLoadingViewHolder(R.layout.shimmer_my_book.makeView(parent = parent))

        ADAPTER_CUBE_EMPTY_VIEW_HOLDER -> CubeEmptyViewHolder(R.layout.item_empty_similar_books.makeView(parent = parent))
