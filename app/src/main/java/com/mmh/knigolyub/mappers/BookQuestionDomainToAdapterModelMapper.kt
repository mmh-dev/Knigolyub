...
class BookQuestionDomainToAdapterModelMapper : Mapper<BookQuestionDomain, QuestionModel> {
    override fun map(from: BookQuestionDomain): QuestionModel = from.run {
        QuestionModel(id = id, a = a, b = b, chapter = chapter, c = c, d = d, question = question, rightAnswer = rightAnswer, bookId = bookId)
    }
}