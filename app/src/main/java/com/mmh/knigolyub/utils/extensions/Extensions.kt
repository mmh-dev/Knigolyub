...
fun Fragment.setTabLayoutColor(tabLayout: TabLayout) {
    when (requireContext().resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) {
        Configuration.UI_MODE_NIGHT_NO -> {
            ...
            tabLayout.setTabTextColors(requireContext().getColor(R.color.white), requireContext().getColor(R.color.white))
        }
...
fun Fragment.createNewPath(inputStream: InputStream, key: String) = App.applicationScope.launch {
    ...
    requireContext().getSharedPreferences(key, Context.MODE_PRIVATE).edit().putString(key, newPath).apply()
}

fun Context.shoErrorDialog(message: String, listener: DialogInterface.OnClickListener) {

    val dialog = AlertDialog.Builder(this).setTitle(R.string.splash_error_title).setCancelable(false).setMessage(message)
        .setPositiveButton(R.string.try_again, listener).create()

    dialog.show()
}

fun Activity.getShPrString(key: String): String? = this.getSharedPreferences(key, Context.MODE_PRIVATE).getString(key, null)
...
fun UserImage.toDto(): UserDomainImage = UserDomainImage(type = type, url = url, name = name)

fun UserDomainImage.toDto(): UserImage = UserImage(type = type, url = url, name = name)
...
fun ParseFile.toImage(): UserImage = UserImage(name = name, type = "File", url = url)

fun ParseFile.toBookImage(): BookPoster = BookPoster(name = name, url = url)

fun ParseFile.toBookDomainImage(): BookPosterDomain = BookPosterDomain(name = name, url = url)

fun ParseFile.toPdf(): BookPdf = BookPdf(name = name, url = url)
...
fun Fragment.showCustomInputAlertDialog(radioButton: RadioButton, text: String) {
    ...
    val dialog = AlertDialog.Builder(requireContext()).setTitle(R.string.question_dilaog_title).setView(dialogBinding.root)
        .setPositiveButton(R.string.action_confirm, null).create()
...
fun Context.glidePlaceHolder(uri: String?, imageView: ImageView) {
    Glide.with(this).load(uri).placeholder(R.drawable.placeholder_avatar).into(imageView)
}
