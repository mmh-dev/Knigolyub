...
class BookThatReadRepositoryImpl(
    ...
    override fun addBook(book: AddNewBookThatReadDomain): Flow<Resource<Unit>> = flow {
        ...
        if (request.status == Status.SUCCESS) {
            val studentBook = book.toStudentBook(objectId = request.data!!.objectId, createdAt = request.data!!.createdAt, path = book.book)
    ...
    override fun updateChapters(
        ...
        emit(Resource.loading())
        val result = cloudDataSource.updateChapters(id = id, chapters = chapters, isReadingPages = isReadingPages)
        if (result.status == Status.SUCCESS) {
