...
interface ClassCloudDataSource {
    ...
    class Base(
        ...
        override suspend fun getAllClass(schoolId: String): Resource<List<ClassData>> {
            ...
            return if (response.status == Status.SUCCESS) Resource.success(data = response.data!!.classes.map { classCloud ->
                mapper.map(classCloud)
            })
            else Resource.error(message = response.message!!)
        }

        override suspend fun deleteClass(id: String): Resource<Unit> = safeApiCall { service.deleteClass(id = id) }
        ...
        override suspend fun addClass(
            ...
            service.addBookQuestion(AddClassCloud(title = title, schoolId = schoolId))
        }
    }
}