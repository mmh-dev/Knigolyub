...
interface BooksThatReadCloudDataSource {
    ...
    class Base(
        ...
        override suspend fun fetchMyBooks(id: String): Resource<List<BookThatReadData>> = try {
            ...
            booksThatReadCloud.body()!!.books.forEach { booksThatRead ->
                ...
                if (bookCloudList.isNotEmpty()) {
                    ...
                    val thatReadBook = BookThatReadMapper.Base(progress = booksThatRead.progress,
        ...
        override suspend fun getMyBook(id: String, userId: String): Resource<Int> {
            val response = safeApiCall { thatReadService.getMyBook(id = "{\"bookId\":\"$id\",\"userId\":\"$userId\"}") }
            return if (response.status == Status.SUCCESS) {
                val bookList = response.data!!.books
                if (bookList.isNotEmpty()) Resource.success(data = bookList[0].progress)
                else Resource.empty()
        ...
        override suspend fun updateProgress(id: String, progress: Int): Resource<Unit> = safeApiCall {
            thatReadService.bookThatReadUpdateProgress(id = id, progress = UpdateProgressCloud(progress = progress))
        }
        ...
        override suspend fun updateChapters(
            ...
            thatReadService.bookThatReadUpdatePages(id = id, chapters = UpdateChaptersCloud(chaptersRead = chapters, isReadingPages = isReadingPages))
        }
