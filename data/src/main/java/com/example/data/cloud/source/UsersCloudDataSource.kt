    ...
    class Base(
        ...
        override suspend fun fetchMyStudents(
            ...
            user.body()!!.users.forEach { userCloud ->
                val response = service.fetchStudentAttributes("{\"userId\":\"${userCloud.objectId}\"}")
                ...
                val attributes = StudentBookMapper.Base(chaptersRead = chapterRead, booksRead = booksRead, progress = progress, booksId = booksId)

                val student = UserMapper.Base(objectId = userCloud.objectId,
                    ...
                    image = UserImageData(name = userCloud.image.name, url = userCloud.image.url, type = userCloud.image.type),
        ...
        override suspend fun updateUser(
            ...
            service.updateUser(sessionToken = sessionToken, id = id, student = userToDataMapper.map(user))
        }
        ...
        override suspend fun addSessionToken(id: String, sessionToken: String): Resource<Unit> = safeApiCall {
            service.addSessionToken(id = id, userSessionToken = SessionTokenCloud(sessionToken = sessionToken), sessionToken = sessionToken)
        }

        override suspend fun getCurrentUser(sessionToken: String): Resource<UserCloud> = try {
            val response = withContext(Dispatchers.IO) { service.getMe(sessionToken = sessionToken) }
