...
interface BooksCloudDataSource {
    ...
    class Base(
        ...
        override suspend fun fetchBooks(schoolId: String): Resource<List<BookData>> {
            val response = safeApiCall() { service.fetchAllBooks(id = "{\"schoolId\":\"${schoolId}\"}") }
            return if (response.status == Status.SUCCESS) {
                val bookData = response.data!!.books.map { bookCloud -> bookCloudMapper.map(bookCloud) }
        ...
        override suspend fun deleteMyBook(id: String): Resource<Unit> = safeApiCall { service.deleteMyBook(id = id) }
        ...
        override suspend fun deleteBook(id: String): Resource<Unit> = safeApiCall { service.deleteBook(id = id) }
        ...
        override suspend fun fetchChapterQuestions(
            ...
            val response = safeApiCall { service.getAllChapterQuestions(id = "{\"bookId\":\"$id\",\"chapter\":\"$chapter\"}") }
            return if (response.status == Status.SUCCESS) {
                val bookData = response.data!!.questions.map { questionCloud ->
        ...
        override suspend fun deleteBookQuestion(id: String): Resource<Unit> = safeApiCall { service.deleteBookQuestion(id = id) }

        override suspend fun updateQuestion(id: String, question: AddBookQuestionData) = safeApiCall {
            service.updateQuestion(question = questionDataMapper.map(question), id = id)
        }
