...
class BooksRepositoryImpl(
    ...
    override fun addNewBook(book: AddNewBookDomain): Flow<Resource<Unit>> = flow {
        ...
        if (result.status == Status.SUCCESS) {
            cacheDataSource.addNewBook(book = book.toBook(id = result.data!!.objectId, createdAt = result.data!!.createdAt))
    ...
    override fun fetchSearchBook(
        ...
        try {
            ...
            allBooks.forEach { book ->
                ...
                if (title.contains(searchText)) searchBooks.add(bookDomainMapper.map(bookCashMapper.map(book)))
            }
