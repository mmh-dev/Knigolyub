...
class UserRepositoryImpl(
    ...
    override fun fetchMyStudents(classId: String): Flow<Resource<List<StudentDomain>>> = flow {
        ...
        if (userCacheList.isEmpty()) {
            val result = cloudDataSource.fetchMyStudents(id = classId, responseType = StudentsResponseType.ClASS_STUDENTS)
            if (result.status == Status.SUCCESS) {
    ...
    override fun fetchSchoolStudents(schoolId: String): Flow<Resource<List<StudentDomain>>> = flow {
        emit(Resource.loading())
        val result = cloudDataSource.fetchMyStudents(id = schoolId, responseType = StudentsResponseType.SCHOOL_STUDENTS)
        if (result.status == Status.SUCCESS) {
            val userDataList = result.data!!
            if (userDataList.isEmpty()) emit(Resource.empty())
            else {
    ...
    override fun fetchClassUsers(classId: String): Flow<Resource<List<StudentDomain>>> = flow {
        emit(Resource.loading())
        val result = cloudDataSource.fetchMyStudents(id = classId, responseType = StudentsResponseType.ClASS_USERS)
        if (result.status == Status.SUCCESS) {
            val userDataList = result.data!!
            if (userDataList.isEmpty()) emit(Resource.empty())
            else {
    ...
    override fun updateStudentClass(
        ...
        emit(Resource.loading())
        val result = cloudDataSource.updateClass(id = id, sessionToken = sessionToken, classId = classId, classTitle = classTitle)
...
enum class StudentsResponseType {
    ClASS_USERS, ClASS_STUDENTS, SCHOOL_STUDENTS
